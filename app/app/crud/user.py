...
class CRUDUser(CRUDBase[User, UserCreate, UserUpdate]):
    def get_by_email(self, db: Session, email: EmailStr) -> User or None:
        return db.query(User).filter(User.email == email).first()

    def create(self, db: Session, obj_in: UserCreate or UserAdminCreate) -> User:
    ...
    def authenticate(self, db: Session, *, email: EmailStr, password: str) -> User or None:
